...
var check = function() {
    ...
    Device.findAllDevices(function(err, devices) {
        ...
        _.each(devices, function(dev) {
            ...
            // 3) - last seen (vpn, browseraddon or logger app) was before the limit OR
            //    - no connections and created before the limit
            if (!dev.inactivity_notif_sent && !dev.removed && 
                ((dev.vpn_last_seen && 
                  dev.vpn_last_seen < inactivelim) ||
                 (dev.browseraddon_lastseen && 
                  dev.browseraddon_lastseen < inactivelim) ||
                 (dev.loggerapp_lastseen && 
                  dev.loggerapp_lastseen < inactivelim) ||
                 (!dev.browseraddon_lastseen &&
                  !dev.loggerapp_lastseen &&
                  !dev.vpn_last_seen &&
                  dev.created < inactivelim))) 
            {
                ...
                var opt = {
                    ...
                    '" has been inactive (i.e. no VPN/loggerapp/browseraddon connections) for more than '+
                    app.get('inactive') +
                    ' days.\n\n' +
